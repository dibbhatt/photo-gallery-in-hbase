...
public class Export implements Runnable {
	...
	private static void extract(HTable table, byte[] key) {
		try {
			long start = System.currentTimeMillis();//.nanoTime(); // start timing
			Get g = new Get(key);
			Result r = table.get(g);
			byte[] value = r.getValue(Import.family, Import.qualifier);
			if (value != null) {
				long stop = System.currentTimeMillis();//.nanoTime(); // stop timing
